...
void setup() {
  ...
  for (int i = 0; i < totalPopulation; i++) {
    dna.add( new DNA( random(width), random(height), (int)random(255), (int)random(255), (int)random(255)) );
  }
}
...
void draw() {
  ...
  if (dna.size()>0) totalFitness /= dna.size();
  println(totalFitness);
  ranWait = (int)map(mouseY,0,height,1,100);
  if (frameCount % ranWait == 0) { 
    nextGeneration();
    //ranWait = (int)map(totalFitness, .6, 1, 1, 100);//(int)random(20,100);
    //if(ranWait < 1) ranWait = 1;
...
void nextGeneration() {
  ...
  for (int i = 0; i < dna.size (); i++) {
    ...
    DNA original = dna.get(i);
    DNA child = partnerA.crossover(partnerB,original);
