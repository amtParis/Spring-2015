...
void main(void){
    ...
    accum = clamp(texture2D(Right, vertTexCoord.st+stereo)*vec4(l1,l2,l2,1.0),0.0,1.0);

	fragment.r = pow(accum.r+accum.g+accum.b, 1.00);
	fragment.a = accum.a;

//	accum = clamp(texture2D(Right, vertTexCoord.st-stereo)*vec4(r2,r1,0.0,1.0),0.0,1.0);
    accum = clamp(texture2D(Left, vertTexCoord.st-stereo)*vec4(r2,r1,0.0,1.0),0.0,1.0);
    ...
    accum = clamp(texture2D(Left, vertTexCoord.st-stereo)*vec4(r2,0.0,r1,1.0),0.0,1.0);
